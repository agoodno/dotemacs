...
#+BEGIN_SRC emacs-lisp
  ...
  (when (file-exists-p user-file)
    (progn
      (message "Loading user env file %s." (symbol-value 'user-file))
      (load user-file)))

  (when (file-exists-p system-file)
    (progn
      (message "Loading system env file %s." (symbol-value 'system-file))
      (load system-file)))

  (when (file-exists-p host-file)
    (progn
      (message "Loading host env file %s." (symbol-value 'host-file))
      (load host-file)))
